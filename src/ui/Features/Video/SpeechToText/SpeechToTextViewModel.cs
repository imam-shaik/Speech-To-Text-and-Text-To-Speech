...
public partial class SpeechToTextViewModel : ObservableObject
{
    ...
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(IsCancelAllVisible))]
    private bool _isTranscribeEnabled;
    ...
    [ObservableProperty] private string _folderScanProgressText;
    [ObservableProperty] private double _folderScanProgressPercent;
    [ObservableProperty] private int _folderScanTotal;
    [ObservableProperty] private int _folderScanCompleted;
    [ObservableProperty] private int _folderScanRemaining;
    [ObservableProperty] private bool _isScanning;
    [ObservableProperty] private bool _doSpecificDuration;
    [ObservableProperty] private TimeSpan _startTime;
    [ObservableProperty] private TimeSpan _endTime;
    [ObservableProperty] private bool _isResumeMode;
    [ObservableProperty] private string _resumeSrtPath = string.Empty;
    [ObservableProperty] private string _lastScannedFolder = string.Empty;
    [ObservableProperty] private ObservableCollection<string> _audioTracks = new();
    [ObservableProperty] private int _selectedAudioTrackIndex = 0;
    ...
    private enum ConflictAction { None, Skip, SkipAll, Overwrite, OverwriteAll }
    ...
    public SpeechToTextViewModel(IWindowService windowService, IFileHelper fileHelper, IFolderHelper folderHelper)
    {
        ...
        FolderScanProgressText = string.Empty;
        _audioTrackNumber = -1;
        _error = string.Empty;
...
#pragma warning restore CA1416
    ...
    private async Task MakeResult(Subtitle? transcribedSubtitle)
    {
        ...
        {
            // Offset transcription if we started from a specific time
            if (DoSpecificDuration)
            {
                foreach (var p in transcribedSubtitle!.Paragraphs)
                {
                    p.StartTime.TotalMilliseconds += StartTime.TotalMilliseconds;
                    p.EndTime.TotalMilliseconds += StartTime.TotalMilliseconds;
                }
            }

            // Merge if in resume mode
            if (IsResumeMode && !string.IsNullOrEmpty(ResumeSrtPath) && File.Exists(ResumeSrtPath))
            {
                var baseSub = new Subtitle();
                try
                {
                    new SubRip().LoadSubtitle(baseSub, new List<string>(File.ReadAllLines(ResumeSrtPath)), ResumeSrtPath);
                    foreach (var p in transcribedSubtitle!.Paragraphs)
                    {
                        baseSub.Paragraphs.Add(p);
                    }
                    transcribedSubtitle = baseSub;
                }
                catch
                {
                    // ignore
        ...
        else if (_abort)
        {
            ...
            if (anyLinesTranscribed)
            {
                ...
                {
                    var srtPath = Path.Combine(Path.GetDirectoryName(videoPath) ?? string.Empty, Path.GetFileNameWithoutExtension(videoPath) + "-breaked.srt");
                    var format = new SubRip();
                    var text = format.ToText(transcribedSubtitle!, string.Empty);
                    File.WriteAllText(srtPath, text);
...
#pragma warning restore CA1416
    ...
    private async Task Add()
    {
        ...
        var countBefore = BatchItems.Count;
        var error = await AddFiles(fileNames);
        var addedCount = BatchItems.Count - countBefore;

        if (error)
        {
            await MessageBox.Show(Window!,
                 "Unable to get video info",
                 "Some files were skipped as video info was unavailable",
             MessageBoxButtons.OK,
        ...
        if (addedCount > 0)
        {
            await MessageBox.Show(Window!, "Finished", $"{addedCount} files added.");
        }
    }
    ...
    private async Task<bool> AddFiles(string[] fileNames)
    {
        ...
        try
        {
            ...
            await Task.Run(async () =>
            {
                ...
                {
                    var srtPath = Path.Combine(Path.GetDirectoryName(fileName) ?? string.Empty, Path.GetFileNameWithoutExtension(fileName) + ".srt");
                    var srtExists = File.Exists(srtPath);
                    var outputSubtitleFileName = string.Empty;

                    if (srtExists && conflictAction != ConflictAction.SkipAll && conflictAction != ConflictAction.OverwriteAll)
                    {
                        var result = await Dispatcher.UIThread.InvokeAsync(async () =>
                        {
                            return await MessageBox.Show(Window!,
                                Se.Language.General.FileAlreadyExists,
                                string.Format(Se.Language.General.FileXAlreadyExists, Path.GetFileName(srtPath)) + Environment.NewLine + Environment.NewLine + "Replace existing file?",
                                MessageBoxButtons.OKCancel,
                                MessageBoxIcon.Question,
                                Se.Language.General.Replace,     // Custom1
                                Se.Language.General.ChangeAll,   // Custom2 (Replace all)
                                Se.Language.General.SkipAll);    // Custom3
                        });

                        conflictAction = result switch
                        {
                            MessageBoxResult.Custom1 => ConflictAction.Overwrite,
                            MessageBoxResult.Custom2 => ConflictAction.OverwriteAll,
                            MessageBoxResult.Custom3 => ConflictAction.SkipAll,
                            MessageBoxResult.OK => ConflictAction.Skip, // OK button used for "Skip"
                            _ => ConflictAction.None // Cancel
                        };
                    ...
                    if (srtExists)
                    {
                        if (conflictAction == ConflictAction.Skip || conflictAction == ConflictAction.SkipAll)
                        {
                            continue;
                        }
                        if (conflictAction == ConflictAction.Overwrite || conflictAction == ConflictAction.OverwriteAll)
                        {
                            outputSubtitleFileName = srtPath;
                        }
                    }
                    ...
                    else
                    {
                        ...
                        {
                            OutputSubtitleFileName = outputSubtitleFileName
                        };
    ...
    {
        var folderPath = await _folderHelper.PickFolderAsync(Window!, Se.Language.General.ScanFolder);
        if (string.IsNullOrEmpty(folderPath) || Window == null)
        {
            return;
        }
    ...
    {
        if (string.IsNullOrEmpty(LastScannedFolder))
        {
            return;
        }
        await ScanFolderInternal(LastScannedFolder);
    }

    private async Task ScanFolderInternal(string folderPath)
    {
        if (Window == null)
        {
            return;
        ...
        Window.Cursor = new Cursor(StandardCursorType.Wait);
        IsScanning = true;
        FolderScanProgressText = Se.Language.General.Scanning;
        FolderScanProgressPercent = 0;
        FolderScanTotal = 0;
        FolderScanCompleted = 0;
        FolderScanRemaining = 0;

        try
        {
            var videoExtensions = Utilities.VideoFileExtensions;
            var allFiles = Directory.GetFiles(folderPath, "*.*", SearchOption.AllDirectories)
                .Where(f => videoExtensions.Contains(Path.GetExtension(f).ToLowerInvariant()))
                .ToArray();

            FolderScanTotal = allFiles.Length;
            FolderScanRemaining = allFiles.Length;
            FolderScanProgressText = string.Format(Se.Language.General.FolderScanProgress, 0, FolderScanTotal);

            if (FolderScanTotal == 0)
            {
                await MessageBox.Show(Window,
                    Se.Language.General.NoVideoFilesFoundInFolder,
                    Se.Language.General.ScanFolder,
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Information);
                IsScanning = false;
                FolderScanProgressText = string.Empty;
            ...
            await Task.Run(async () =>
            {
                ...
                foreach (var fileName in allFiles)
                {
                    ...
                    processedCount++;
                    var pct = (double)processedCount / FolderScanTotal * 100;
                    await Dispatcher.UIThread.InvokeAsync(() =>
                    {
                        FolderScanProgressPercent = pct;
                        FolderScanCompleted = processedCount;
                        FolderScanRemaining = FolderScanTotal - processedCount;
                        FolderScanProgressText = string.Format(Se.Language.General.FolderScanProgress, processedCount, FolderScanTotal);
                    });
                }
            });

            if (error && processedCount > 0 && BatchItems.Count == 0)
            {
                await MessageBox.Show(Window!,
                    "Unable to get video info from files",
                    "Scan Folder",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);
            }
    ...
    private void CancelAll()
    {
        _abort = true;
        _abortBatch = true;
        LogToConsole("Batch process cancelled by user.");
    }
    ...
    private async Task Resume()
    {
        ...
        var files = await _fileHelper.PickOpenSubtitleFiles(Window, "Select broken SRT file", false);
        if (files != null && files.Length > 0)
        {
            ...
            try
            {
                new SubRip().LoadSubtitle(sub, new List<string>(File.ReadAllLines(srtPath)), srtPath);
                if (sub.Paragraphs.Count > 0)
                {
                    var last = sub.Paragraphs.Last();
                    StartTime = last.EndTime.TimeSpan;
                    DoSpecificDuration = true;
                    IsResumeMode = true;
                    ResumeSrtPath = srtPath;
                    LogToConsole($"Resume mode enabled: starting from {last.EndTime}");
                }
            }
            catch (Exception ex)
            {
                await MessageBox.Show(Window, "Error loading subtitle", ex.Message);
            }
        }
    }
...
#pragma warning restore CA1416
    ...
    private Process? GetFfmpegProcess(string videoFileName, int audioTrackNumber, string outWaveFile)
    {
        ...
        var trackIndex = IsBatchMode ? audioTrackNumber : SelectedAudioTrackIndex;
        if (trackIndex >= 0)
        {
            audioParameter = $"-map 0:a:{trackIndex}";
        }

        var timeRange = string.Empty;
        if (DoSpecificDuration)
        {
            timeRange = $"-ss {StartTime.TotalSeconds} -to {EndTime.TotalSeconds}";
            if (EndTime <= StartTime || EndTime.TotalSeconds <= 0)
            {
                // If end time is not set, just use -ss to the end
                timeRange = $"-ss {StartTime.TotalSeconds}";
            }
        }
        ...
        var parameters = string.Format(fFmpegWaveTranscodeSettings, videoFileName, outWaveFile, audioParameter + " " + timeRange);
        return new Process
        {
    ...
    internal void Initialize(string? videoFileName, int audioTrackNumber)
    {
        ...
        {
            var mediaInfo = FfmpegMediaInfo.Parse(_videoFileName);
            var tracks = mediaInfo.Tracks.Where(p => p.TrackType == FfmpegTrackType.Audio).ToList();
            AudioTracks.Clear();
            for (int i = 0; i < tracks.Count; i++)
            {
                var track = tracks[i];
                AudioTracks.Add($"Track {i + 1}: {track.TrackInfo}");
            }
            if (audioTrackNumber >= 0 && audioTrackNumber < AudioTracks.Count)
            {
                SelectedAudioTrackIndex = audioTrackNumber;
            }
